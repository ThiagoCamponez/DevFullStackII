...
export default class AtividadeSustentavel {
    ...
    async init() {
        try 
        {
            ...
            CREATE TABLE IF NOT EXISTS atividadeSustentavel(
                ...
                CONSTRAINT fk_tipoAtividade FOREIGN KEY(tipo_id) REFERENCES tipoAtividadeSustentavel(tipo_id)
            );
    ...
    async gravar(atividadeSustentavel) {
        if (atividadeSustentavel instanceof AtividadeSustentavel) {
            const sql = `INSERT INTO atividadeSustentavel(ativ_id, ativ_nome, ativ_cpf, ativ_contato, ativ_endereco, ativ_bairro, ativ_numero, tipo_id, ativ_data, ativ_horarioIncial, ativ_horarioFinal, ativ_descricaoCompleta)	
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`;
    ...
    async atualizar(atividadeSustentavel) {
        if (atividadeSustentavel instanceof AtividadeSustentavel) {
            ...
            ativ_contato = ?, ativ_endereco = ?, ativ_bairro = ?, ativ_numero = ?, tipo_id = ?, tipo_data = ?, ativ_horarioIncial = ?, ativ_horarioFinal = ?, ativ_descricaoCompleta = ? WHERE ativ_id = ?`;
    ...
    async consultar(termo) {
        ...
        if (!isNaN(parseInt(termo))){
            ...
            const sql = `SELECT p.ativ_id, p.ativ_nome, p.ativ_cpf, p.ativ_contato, 
                         p.ativ_endereco, p.ativ_bairro, p.ativ_numero, p.tipo_id, p.tipo_data, p.ativ_horarioIncial, p.ativ_horarioFinal, p.ativ_descricaoCompleta
                         FROM atividadeSustentavel p 
                         INNER JOIN tipoAtividadeSust t ON p.ativ_id = t.ativ_id
                         WHERE p.ativ_id = ?
        ...
        else
        {
            ...
            for (const registro of registros){
                ...
                listaProdutos.push(atividadeSustentavel);
            }
        }
